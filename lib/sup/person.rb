module Redwood
  ...
  class Person
    attr_accessor :name, :email, :default_from

    def initialize(name, email, default_from=nil)
      raise ArgumentError, "email can't be nil" unless email

      email.fix_encoding!
      default_from.fix_encoding! if default_from
      ...
      @default_from = default_from.strip.gsub(/\s+/, " ") if default_from
    end
    ...
    class << self
      ...
      def from_address s
        ...
        name, email = case s
                      when /(.+?) ((\S+?)@\S+) \3/
                        ## ok, this first match case is insane, but bear with me.  email
